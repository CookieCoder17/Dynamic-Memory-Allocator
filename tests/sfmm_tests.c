...
Test(sfmm_student_suite, student_test_1, .timeout = TEST_TIMEOUT) {
	double* ptr = sf_malloc(3000);
    *ptr = 10.00;
    double* ptr2 = sf_malloc(32);
    *ptr2 = 20.00;
    double* ptr3 = sf_malloc(1);
    *ptr3 = 20.00;
    sf_free(ptr);
    sf_free(ptr3);
...
Test(sfmm_student_suite, student_test_2, .timeout = TEST_TIMEOUT) {
    ...
    *ptr3 = 20.00;
    ...
    assert_free_list_size(0,0);
    assert_free_list_size(9,1);
    assert_free_list_size(8,0);
    assert_free_block_count(4048, 1);
    assert_free_block_count(976, 0);
}

Test(sfmm_student_suite, student_test_3, .timeout = TEST_TIMEOUT) {
	cr_assert_eq(sf_fragmentation(),0.0);
	cr_assert_eq(sf_utilization(),0.0);
    double* ptr = sf_malloc(4032);
    for(int i = 0; i < NUM_FREE_LISTS; i++){
	    assert_free_list_size(i,0);
    }
    cr_assert_eq(sf_fragmentation(),0.9960474308300395485);
	cr_assert_eq(sf_utilization(),0.984375);
    sf_free(ptr);
    assert_free_list_size(9,1);
	cr_assert_eq(sf_fragmentation(),0.0);
	cr_assert_eq(sf_utilization(),0.984375);
...
Test(sfmm_student_suite, student_test_4, .timeout = TEST_TIMEOUT) {
    ...
    cr_assert_eq(sf_fragmentation(),0.66666666666666662966);
	cr_assert_eq(sf_utilization(),0.0078125);
	sf_realloc(ptr, 48);
	cr_assert_eq(sf_fragmentation(),0.75);
	cr_assert_eq(sf_utilization(),0.01953125);
}

Test(sfmm_student_suite, student_test_5, .timeout = TEST_TIMEOUT) {
    double* ptr = sf_malloc(4032);
    double *ptr2 = sf_malloc(4030);
